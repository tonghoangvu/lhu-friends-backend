...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<Mono<ErrorResponse>> handleAppException(AppException e) {
        ErrorResponse errorResponse = new ErrorResponse(e.getCode(), e.getMessage());
        return ResponseEntity.status(e.getStatus()).body(Mono.just(errorResponse));
    }

    @ExceptionHandler(Exception.class)
    protected ResponseEntity<Mono<ErrorResponse>> handleUnwantedException(Exception e) {
